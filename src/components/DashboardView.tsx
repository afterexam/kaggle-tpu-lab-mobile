...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const BADGES: Record<string, { label: string; color: string; bg: string }> = {
    WINNER: { label: '🏆 Won', color: 'var(--accent-emerald)', bg: 'rgba(16, 185, 129, 0.2)' },
    RUNNING: { label: 'Running', color: 'var(--accent-cyan)', bg: 'rgba(6, 182, 212, 0.2)' },
    QUEUED: { label: 'Queued', color: 'var(--accent-amber)', bg: 'rgba(245, 158, 11, 0.2)' },
    CANCELLED: { label: 'Cancelled', color: 'var(--text-muted)', bg: 'rgba(100, 116, 139, 0.15)' },
    COMPLETE: { label: 'Completed', color: 'var(--accent-blue)', bg: 'rgba(59, 130, 246, 0.2)' },
  ...
  return (
    <div style={{ paddingBottom: '30px' }}>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--accent-emerald)', fontWeight: 700 }}>
              ...
              <span>
                {endpoints.length === 1
                  ? `TPU Endpoint Online (${endpoints[0].accountName || 'Ready'})`
                  : `${endpoints.length} TPU instances healthy & ready`}
              </span>
            </div>
          ...
          >
            ...
            <div style={{ display: 'flex', gap: '4px', flexShrink: 0 }}>
              ...
              >
                ...
                <span style={{ fontSize: '10px', marginLeft: '3px' }}>{copiedText === 'top-url' ? 'Copied' : 'URL'}</span>
              </button>
              <button
                className="btn btn-secondary btn-sm"
                style={{ padding: '2px 6px', fontSize: '11px' }}
              ...
              >
                ...
                <span style={{ fontSize: '10px', marginLeft: '3px' }}>{copiedText === 'top-key' ? 'Copied' : 'Key'}</span>
              </button>
            </div>
          </div>
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            ...
            >
              ...
              <span>{copiedText === 'top-bundle' ? 'Copied URL+Key' : 'Copy URL + Key'}</span>
            </button>
      ...
      >
        ...
        >
          <Layers size={15} />
          <span>Independent Key Monitor</span>
        </button>
      ...
      <div className="glass-card">
        <div className="card-title">
          ...
          <span>Model & Parameters (TPU v5e-8)</span>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginTop: '10px' }}>
          <div className="form-group" style={{ marginBottom: 0 }}>
            ...
            >
              <option value="xhigh">xhigh (Extreme · Qwen default)</option>
              <option value="high">high (Deep)</option>
              <option value="medium">medium (Moderate)</option>
              <option value="low">low (Fast · GLM default)</option>
            </select>
          </div>

          <div className="form-group" style={{ marginBottom: 0 }}>
            <label className="form-label">Fast Start Mode</label>
            <select
              className="form-select"
              value={config.fastStart ? 'true' : 'false'}
              onChange={(e) => onUpdateConfig({ ...config, fastStart: e.target.value === 'true' })}
            >
              <option value="false">Standard Compilation (Safer)</option>
              <option value="true">Skip Warmup (~4min faster)</option>
            </select>
          </div>
        </div>
      </div>
        ...
        <div>
          ...
          >
            <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-secondary)' }}>
              Key Status Dashboard ({accounts.length} Accounts)
            </span>
            {anyRunning && (
              <button
          ...
          {accounts.length === 0 ? (
            <div className="glass-card" style={{ textAlign: 'center', padding: '30px' }}>
              ...
              <div style={{ fontWeight: 600, marginBottom: '6px' }}>No Kaggle Accounts Configured</div>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>
                Please go to the Settings tab below to add your Kaggle API Token.
              </div>
            </div>
          ) : (
            accounts.map((acc) => {
                ...
                >
                  ...
                  {session && session.events.length > 0 ? (
                    ...
                    >
                      <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--text-muted)', marginBottom: '4px' }}>
                        <span>Latest Update ({session.model || config.model})</span>
                      ...
                      {session.endpoint && session.endpoint.status === 'READY' ? (
                        ...
                        >
                          ...
                          <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                            ...
                            >
                              ...
                              <span>{copiedText === `card-bundle-${acc.id}` ? 'Copied' : 'Copy URL+Key'}</span>
                            </button>
                            <button
                              className="btn btn-secondary btn-sm"
                            ...
                            >
                              ...
                              <span>{copiedText === `card-url-${acc.id}` ? 'Copied' : 'Copy URL'}</span>
                            </button>
                            <button
                              className="btn btn-secondary btn-sm"
                              style={{ padding: '3px 8px', fontSize: '11px' }}
                            ...
                            >
                              ...
                              <span>{copiedText === `card-key-${acc.id}` ? 'Copied' : 'Copy Key'}</span>
                            </button>
                            <button
                        ...
                        >
                          ...
                          <div style={{ display: 'flex', gap: '6px' }}>
                            ...
                            >
                              {copiedText === `card-key-${acc.id}` ? <Check size={12} color="var(--accent-emerald)" /> : <Copy size={12} />}
                              <span>{copiedText === `card-key-${acc.id}` ? 'Copied' : 'Copy API Key'}</span>
                            </button>
                          </div>
                        </div>
                      ) : null}
                    ...
                    <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '10px' }}>
                      {acc.isValid ? 'Instance is idle and ready to launch' : 'Account not verified. Please test and save in Settings.'}
                    </div>
                  )}
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    >
                      ...
                      <span>{refreshingAccounts[acc.id] ? 'Syncing...' : 'Sync'}</span>
                    </button>
                    ...
                    {session && session.events.length > 0 && (
                      <button
                        ...
                        title="Expand/collapse event log"
                      >
                        {isExpanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                        <span>Logs ({session.events.length})</span>
                      </button>
                    )}
                  </div>
        ...
        <div>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--accent-amber)', fontWeight: 700, marginBottom: '6px' }}>
              <Zap size={18} />
              <span>Multi-Account Preemptive Race Mechanism</span>
            </div>
            <div style={{ fontSize: '12px', color: 'var(--text-secondary)', lineHeight: '1.5' }}>
              Selected Kaggle accounts will queue concurrently for TPU allocation in the cloud.
              <strong> As soon as any account becomes ready or enters RUNNING, all other queuing accounts are automatically cancelled</strong>,
              safely guarding your 20h weekly free TPU quota without waste!
            </div>
          </div>
          ...
          <div className="glass-card" style={{ marginBottom: '14px' }}>
            <div className="card-title" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span>Select Accounts for Race ({selectedRaceIds.length} selected)</span>
              <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>Select at least 2</span>
            </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>
                          @{acc.username || 'No username configured'} · {acc.isValid ? 'Verified' : 'Unverified'}
                        </div>
                      </div>
                    </div>
              ...
              >
                ...
                <span>
                  {selectedRaceIds.length < 2
                    ? 'Select at least 2 valid accounts to start race'
                    : `⚡ Launch Race with ${selectedRaceIds.length} Accounts`}
                </span>
              </button>
            )}
          </div>
          ...
          {sessions.some((s) => !!s.raceGroupId) && (
            <div className="glass-card">
              <div className="card-title">
                ...
                <span>Race Timeline & Status</span>
              </div>
