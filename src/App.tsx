...
export const App: React.FC = () => {
      ...
      try {
        ...
        const normalizeAccountName = (name: any, index: number): string => {
          if (!name || typeof name !== 'string') {
            return index === 0 ? 'Primary Account' : `Account ${index + 1}`;
          }
  ...
  const getStatusInfo = () => {
    if (endpoints.length > 0) {
      return { text: `${endpoints.length} Ready`, cls: 'ready' };
    }
    if (sessions.some((s) => !s.done)) {
      ...
      <header className="app-header">
        <div className="brand-badge">
          ...
          <div>
            ...
            <div className="brand-sub">Cloud TPU v5e-8 Multi-Key Controller</div>
          </div>
        </div>
