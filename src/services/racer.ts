...
export class InstanceManager {
      ...
      events: [
        {
          ...
          phase: 'pushing',
          text: `${prefix}Pushing ${config.model} task to [${account.username || account.name}]...`,
        },
      ],
    };
    ...
    try {
      ...
      if (pushRes.error && (pushRes.error.includes('running') || pushRes.error.includes('Conflict') || pushRes.error.includes('adopted') || pushRes.error.includes('already'))) {
        session.events.push({
          time: new Date().toLocaleTimeString(),
          phase: 'sync',
          text: `${prefix}Active TPU instance detected in cloud, adopting and syncing endpoint config...`,
        });
        ...
        session.events.push({
          time: new Date().toLocaleTimeString(),
          phase: 'queued',
          text: `${prefix}Push successful, waiting in queue for TPU slot allocation...`,
        });
      }
    } catch (err: any) {
      ...
      session.events.push({
        time: new Date().toLocaleTimeString(),
        phase: 'failed',
        text: `Push failed: ${err.message || String(err)}`,
      });
    }
  ...
  public async startRace(accounts: KaggleAccount[], config: LaunchConfig): Promise<void> {
    ...
    if (valid.length < 2) throw new Error('Race mode requires selecting at least 2 valid Kaggle accounts');
      ...
      try {
        ...
        if (st.status === 'RUNNING' && session.status !== 'RUNNING' && session.status !== 'WINNER') {
          ...
          session.events.push({
            time: new Date().toLocaleTimeString(),
            phase: 'running',
            text: `[${session.username || session.accountName}] TPU instance allocated, initializing environment...`,
          });
    ...
    if (!foundActive) {
      if (session) {
        ...
        this.sessions.set(account.id, {
          ...
          events: [{ time: new Date().toLocaleTimeString(), phase: 'sync', text: 'Cloud synced, no running or queued TPU tasks found for this account' }],
        });
      }
    }
