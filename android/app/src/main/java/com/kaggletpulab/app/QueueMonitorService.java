...
public class QueueMonitorService extends Service {
    ...
    private void createChannels() {
        ...
        NotificationChannel queue = new NotificationChannel(
                CHANNEL_QUEUE, "TPU queue monitor", NotificationManager.IMPORTANCE_LOW);
        queue.setDescription("Persistent status while waiting for a TPU slot");
        NotificationChannel ready = new NotificationChannel(
                CHANNEL_READY, "TPU ready alerts", NotificationManager.IMPORTANCE_HIGH);
        ready.setDescription("Fires the moment your TPU kernel is ready");
    ...
    private Notification buildQueueNotification() {
        ...
        return new NotificationCompat.Builder(this, CHANNEL_QUEUE)
                ...
                .setContentText("Monitoring " + count + " TPU queue" + (count > 1 ? "s" : "") + " in background…")
                .setOngoing(true)
    ...
    private void fireReadyNotification(String topic, String model, String endpoint) {
        ...
        if (!model.isEmpty()) body.append("Model ").append(model).append(" is online. ");
        if (!endpoint.isEmpty()) body.append(endpoint);
        if (body.length() == 0) body.append("Tap to open the app.");
        Notification n = new NotificationCompat.Builder(this, CHANNEL_READY)
                .setSmallIcon(getApplicationInfo().icon)
                .setContentTitle("\uD83C\uDF89 TPU Service Ready!")
    ...
    private void fireTimeoutNotice() {
        Notification n = new NotificationCompat.Builder(this, CHANNEL_READY)
                .setSmallIcon(getApplicationInfo().icon)
                .setContentTitle("Queue monitoring paused")
                .setContentText("Android stopped background monitoring (system time limit). Reopen the app to resume.")
                .setPriority(NotificationCompat.PRIORITY_HIGH)
                .setAutoCancel(true)
                .setContentIntent(openAppIntent())
                .build();
